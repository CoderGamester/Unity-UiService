...
- **Composable features**: `Runtime/Features/*`
  ...
  - `UiToolkitPresenterFeature.PanelSettings` is a **read-only** accessor. There is deliberately no per-presenter `PanelSettings` feature: a `PanelSettings` asset *is* a panel, documents sharing one share a panel (which is what makes `UIDocument.sortingOrder` ordering work), and runtime mutation of a shared `ScriptableObject` reconfigures every other presenter using it. Presenters needing different settings get a **second authored `PanelSettings` asset**; world-space UI Toolkit uses `PanelSettings.renderMode = PanelRenderMode.WorldSpace` + per-document `UIDocument.position`/`pivot`/`worldSpaceSize`, not a render texture.
- **URP presenter features**: `Runtime/Rendering/*` (namespace `GameLovers.UiService.Rendering`) compile into the **main `GameLovers.UiService` assembly** — this package is URP-only, so the asmdef references `Unity.RenderPipelines.Universal.Runtime` + `Unity.RenderPipelines.Core.Runtime` directly. There is no second assembly: a hard URP dependency in `package.json` plus a quarantine assembly would be paying the cost of an optional dependency while keeping the restriction of a required one.
  - `UiCameraStackFeature`: Screen Space - Camera + URP `cameraStack` insertion for a prefab-child overlay camera. Base camera resolved via a swappable `Func<Camera> BaseCameraResolver` (default `Camera.main` — measured at ~16ns/call and resolved twice per open/close, so do **not** reintroduce a caching provider; the previous one bought nothing and leaked a static `SceneManager.sceneLoaded` subscription). Ordering math lives in the URP-free `UiCameraStackRegistry.InsertIndex`; per-camera priority lives in a private static `Dictionary<Camera, int>` on the feature, reset via `[RuntimeInitializeOnLoadMethod]` exactly like `UiBackdropBlurRequests`. Do **not** switch this to deriving priority from the hierarchy (`GetComponentInParent`): the serialized overlay camera is not required to be a child of the presenter, so that silently yields priority 0 for a camera assigned anywhere else — a real ordering bug caught by `MultipleStackedCameras_OrderByCanvasSortingOrder`.
  - **No render-texture feature, deliberately.** Targeting UI at a `RenderTexture` is fully inspector-authorable in both UI systems — verified serialized properties: `Camera.m_TargetTexture` + `Canvas.m_RenderMode` for uGUI, `PanelSettings.m_TargetTexture` for UI Toolkit. A `UiRenderTextureFeature` existed briefly and was removed: ~180 lines wrapping three inspector fields. Do not re-add one; document the authoring recipe instead.
...
- **Tests**: `Tests/`
  ...
  - **Test asmdef references for `TMPro` / `UnityEngine.UI` are not transitive**: `GameLovers.UiService.asmdef` (Runtime) references `Unity.TextMeshPro` (`6055be8ebefd69e48b49212b09b47b2f`) and `UnityEngine.UI` is auto-referenced via `noEngineReferences: false`, but `GameLovers.UiService.Tests.PlayMode.asmdef` (test) does **not** inherit those references. When extending tests that touch the `TMPro` / `UnityEngine.UI` namespaces directly (e.g. `InteractableTextView` link-click via `TMP_LinkInfo`, `NonDrawingView` via `VertexHelper`), the test asmdef MUST add `Unity.TextMeshPro` (`6055be8ebefd69e48b49212b09b47b2f`) AND `UnityEngine.UI` (`2bafac87e7f4b9b418d9448d219b01ab`) directly. Symptom of getting this wrong: `CS0246: type or namespace 'TMPro' could not be found` from inside the test asmdef even though the same `using TMPro;` compiles in `Runtime/Views/InteractableTextView.cs`.
...
- **Never mutate `PanelSettings` at runtime, and don't add a feature that does**
  ...
  - This package previously shipped a `UiPanelSettingsFeature` that cloned `PanelSettings` per presenter and mirrored `sortingOrder` to compensate. It was removed: it existed only to serve a UI Toolkit render-texture path that `PanelRenderMode.WorldSpace` supersedes, it required a second component on every UI Toolkit prefab, and per-presenter panels work against UI Toolkit's batching model. **Author additional `PanelSettings` assets instead** — a designer-time decision, and the intended usage.
