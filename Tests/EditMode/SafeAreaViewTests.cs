...
namespace GameLovers.UiService.Tests
{
	...
	{
		[Test]
		// ADMIT: SafeAreaMath.ComputeInsets must flip the bottom-origin safe area into top-origin UI Toolkit padding and divide every edge by the panel scale.
		// A2: SafeAreaMath.cs ComputeInsets — remove the scaledPixelsPerPoint division from `top` and this test fails.
		public void ComputeInsets_NotchedLandscape_MapsAllEdgesAtPanelScale()
		{
			var safeArea = new Rect(120f, 40f, 1680f, 1000f);

			SafeAreaMath.ComputeInsets(safeArea, 1920, 1080, 2f, out float left, out float top, out float right, out float bottom);
		...
		[Test]
		// ADMIT: SafeAreaMath.ComputeNormalizedAnchors must map all four safe-area edges into full-screen normalized anchors.
		// A2: SafeAreaMath.cs ComputeNormalizedAnchors — use safeArea.xMax for anchorMin.x and this test fails.
		public void ComputeNormalizedAnchors_NotchedLandscape_MapsAllFourEdges()
		{
			var safeArea = new Rect(141f, 63f, 2496f, 1221f);

			SafeAreaMath.ComputeNormalizedAnchors(safeArea, 2778, 1284, out Vector2 anchorMin, out Vector2 anchorMax);
		...
		[Test]
		// ADMIT: SafeAreaMath.HasScreenChanged must notice a safe-area change when resolution is unchanged, or runtime polling misses simulator posture changes.
		// A2: SafeAreaMath.cs HasScreenChanged — remove `safeArea != appliedSafeArea` and this test returns false.
		public void HasScreenChanged_SameResolutionDifferentSafeArea_ReturnsTrue()
		{
			...
			bool changed = SafeAreaMath.HasScreenChanged(current, 1920, 1080, previous, 1920, 1080);
