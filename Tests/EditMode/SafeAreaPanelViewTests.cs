...
namespace GameLovers.UiService.Tests
{
	...
	{
		[Test]
		// ADMIT: SafeAreaPanelView.ComputeNormalizedAnchors must map all four safe-area edges, not a stretched-path xMin/yMax pair.
		// RCR: SafeAreaPanelView.cs ComputeNormalizedAnchors — use xMax for anchorMin.x → RED (Expected: 0.0507559404f But was: 0.949244082f). 2026-09-01
		public void ComputeNormalizedAnchors_NotchedLandscape_MapsAllFourEdges()
		{
			...
			SafeAreaPanelView.ComputeNormalizedAnchors(safeArea, width, height, out Vector2 anchorMin, out Vector2 anchorMax);
