...
- Added `UiRenderTextureFeature`: renders a presenter's **uGUI `Canvas`** into a `RenderTexture`, switching Screen Space - Overlay to Screen Space - Camera with a dedicated render camera. Defaults to a pre-authored `RenderTexture` asset; dynamic allocation is the escape hatch, tracked via an explicit ownership flag. Deliberately has no UI Toolkit path — `PanelRenderMode.WorldSpace` renders a `UIDocument` in world space natively, with no texture round-trip and no shared-asset mutation
